...
- fmt (`modules/flake-parts/fmt.nix`)
  ...
  - Enables formatters: Alejandra (Nix), Biome (JS/TS), HuJSON, latexindent, mdformat (MD, incl. frontmatter/GFM/footnote/MyST plugins), Ruff (check + format), Rustfmt, Shfmt (shell), Taplo (TOML), Yamlfmt.
...
- pulumi (`modules/flake-parts/pulumi.nix`)
  ...
  - When enabled, Pulumi shells export `PULUMI_OPTION_COLOR=never` and `PULUMI_OPTION_SUPPRESS_PROGRESS=true` for clean CLI output, plus `NODE_OPTIONS=--async-context-frame` for better Node/Pulumi async stack traces. The CI shell (`ci-pulumi`) additionally sets `PULUMI_OPTION_NON_INTERACTIVE=true`; local devshells omit it so interactive commands work.
  - Declared native plugins (`perSystem.jackpkgs.pulumi.plugins`) are `ln -sfn`'d into `$PULUMI_HOME/plugins/<kind>-<name>-v<ver>/` on every shell entry. Indirect Nix GC roots for those store paths live **in the checkout**, not under `$PULUMI_HOME`: `$PRJ_ROOT/.direnv/pulumi-gcroots/<kind>-<name>-v<ver>` (falling back to `FLAKE_ROOT`, the `flake-root` executable, then `git rev-parse --show-toplevel`). That keeps two worktrees pinning different versions from pruning each other; deleting or `git worktree remove`'ing a checkout drops its roots automatically. Each shellHook reconciles that directory to **exactly this checkout's declared plugin set** (multiple declared versions are kept). If `--realise` fails for a plugin, prune for that kind+name is skipped so a substituter outage cannot drop a still-valid previous root. If no project root can be determined, the hook warns and still creates the Pulumi plugin symlink. Leftover `$PULUMI_HOME/plugins/*-v*.gcroot` files from earlier revisions of this fix are **not** deleted automatically (another worktree still on that revision may still need them); after every checkout is upgraded, remove them with `rm -f "${PULUMI_HOME:-$HOME/.pulumi}/plugins/"*-v*.gcroot`. `.direnv/` is gitignored by jackpkgs itself, the default template, and `defaultExcludes`.
  - Options under `jackpkgs.pulumi`:
    ...
    - `plugins` (list of `{ name, kind, version, package }`, per-system, default `[]`) - Native plugins to link into `$PULUMI_HOME/plugins` with checkout-local GC roots as above
