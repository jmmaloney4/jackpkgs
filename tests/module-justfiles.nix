...
in {
  ...
  testNodejsUpdatePnpmHash = mkJustParseTest "nodejs-update-pnpm-hash" ''
    ...
    update-pnpm-hash:
        ...
        echo "🔨 Building pnpm deps FOD to fetch new hash..."
        nix build ".#pnpm-deps" --no-link >"$build_log" 2>&1 || true
        ...
        if [ -z "$new_hash" ]; then
            ...
            if grep -Eq 'Killed: 9|exit code 137' "$build_log"; then
                echo "⚠️  The pnpm fetcher was SIGKILLed. On macOS this is the broken worker_threads"
                echo "⚠️  fd tracking in nixpkgs nodejs_24 24.15.0 (nixpkgs#525627) — check which node"
                echo "⚠️  runtime jackpkgs.nodejs.pnpmPackage resolves to."
            fi
            echo "Output was:"
            cat "$build_log"
            exit 1
