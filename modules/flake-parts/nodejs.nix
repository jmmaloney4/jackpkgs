          ...
          if [ -f "package-lock.json" ]; then
            ...
            ${pkgs.writeScript "validate-package-lock" ''
              ...
              for pkg in $PACKAGES; do
                ...
                LINK=$(jq -r ".packages[\"$pkg\"].link // false" "$LOCKFILE" 2>/dev/null || echo "false")

                # Skip workspace packages (they have link:true or no resolved/integrity)
                if [ "$LINK" = "true" ]; then
                  continue
                fi
                ...
                if echo "$RESOLVED" | grep -qE '^(git\+https://|git\+ssh://|git://)'; then
                  ...
                  echo "  Suggestion: Replace with npm registry version or publish to a private registry."
                  ERRORS=$((ERRORS + 1))
                  continue
                fi
            ...
            ${pkgs.jq}/bin/jq --version || echo "jq not found" 2>&1
          else
