        ...
        jackpkgs.outputs.pulumiJustfile = let
          ...
          previewRecipe =
            ...
            ([
                ...
                "    local stack_name=\"\$2\""
                "    local preview_output create_count replace_count update_count delete_count"
                ""
                "    preview_output=$(\"${pulumiExe}\" -C \"\$project_path\" preview --stack \"\$stack_name\" 2>&1)"
                "    printf '%s\\n' \"\$preview_output\""
                ""
                "    # Extract counts from lines like '    + 3 to create'"
                "    create_count=$(printf '%s\\n' \"\$preview_output\" | sed -n 's/.*\\([0-9][0-9]*\\) to create.*/\\1/p' | tail -1)"
                "    replace_count=$(printf '%s\\n' \"\$preview_output\" | sed -n 's/.*\\([0-9][0-9]*\\) to replace.*/\\1/p' | tail -1)"
                "    update_count=$(printf '%s\\n' \"\$preview_output\" | sed -n 's/.*\\([0-9][0-9]*\\) to update.*/\\1/p' | tail -1)"
                "    delete_count=$(printf '%s\\n' \"\$preview_output\" | sed -n 's/.*\\([0-9][0-9]*\\) to delete.*/\\1/p' | tail -1)"
